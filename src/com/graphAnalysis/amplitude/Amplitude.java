...
public class Amplitude {
	...
	private void query(String symbol) {
		...
		String beginSql = " select a.* from(select valueMax*100 value,'最大值' groupType, id60 from  sa_feature_" + symbol
				+ "   union all select valueMin*100 value,'最小值' groupType, id60 from  sa_feature_" + symbol + " ) a,   "+ "price_" + symbol +"60   b  " + " where a.id60=b.id and "
		...
		// 频率图表
		{
			...
			ChartPanel barPanel = new Bar("振幅分析", "频率值", "类型", dataset).getChart();
