 ...
 * @author:fgq
 */
public class EconomicNewsTab {
	private static EconomicNewsTab tab;
	...
	public static EconomicNewsTab getInstance() {
		if (tab == null)
			tab = new EconomicNewsTab();
		return tab;
	}

	public EconomicNewsTab() {
	...
	private void query() {
		...
		beginSql = " select  * from economic_news where  occurDate >='" + begDate + "' and occurDate <='" + endDate + "'";
		...
		String groupSql = " order by occurDate desc ,occurTime asc";
		sql = beginSql + queryTimesSql + groupSql;

		{
			scrlTabel.setViewportView(new EconomicNewsTable(sql).getJtable());
		}
	}
}
