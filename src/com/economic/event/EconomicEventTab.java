 ...
 * @author:fgq
 */
public class EconomicEventTab {
	private static EconomicEventTab tab;
	...
	public static EconomicEventTab getInstance() {
		if (tab == null)
			tab = new EconomicEventTab();
		return tab;
	}

	public EconomicEventTab() {
	...
	private void query() {
		...
		if ("全部".equals(country)) {
			beginSql = " select  * from economic_event where  occurDate >='" + begDate + "' and occurDate <='" + endDate + "'";
		} else {
			beginSql = " select  * from economic_event where country like'%" + country + "%' and occurDate >='" + begDate + "' and occurDate <='" + endDate + "'";
		}
		...
		{
			scrlTabel.setViewportView(new EconomicEventTable(sql).getJtable());
		}
	}
}
