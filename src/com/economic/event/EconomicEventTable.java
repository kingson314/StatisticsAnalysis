...
public class EconomicEventTable {
	private JTable jtable;

	// 构造
	public EconomicEventTable(String sql) {
		try {
			String[] arr = new String[] { "发生日期", "发生时间", "国家", "地区", "重要性", "事件" };
			Vector<String> columnName = UtilConver.arrayToVector(arr);
			Vector<?> tableValue = EconomicEventDao.getInstance().getVector(sql);
			int[] cellEditableColumn = null;
			int[] columnWidth = new int[] { 100, 60, 60, 100, 50, 800 };
			int[] columnHide = null;
