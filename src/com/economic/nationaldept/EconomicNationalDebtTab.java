 ...
 * @author:fgq
 */
public class EconomicNationalDebtTab {
	private static EconomicNationalDebtTab tab;
	...
	public static EconomicNationalDebtTab getInstance() {
		if (tab == null)
			tab = new EconomicNationalDebtTab();
		return tab;
	}

	public EconomicNationalDebtTab() {
	...
	private void query() {
		...
		if ("全部".equals(country)) {
			beginSql = " select  * from economic_NationalDebt where  occurDate >='" + begDate + "' and occurDate <='" + endDate + "'";
		} else {
			beginSql = " select  * from economic_NationalDebt where country like'%" + country + "%' and occurDate >='" + begDate + "' and occurDate <='" + endDate + "'";
		}
		...
		{
			scrlTabel.setViewportView(new EconomicNationalDebtTable(sql).getJtable());
		}
	}
}
