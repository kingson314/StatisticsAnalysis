...
import com.economicAnalysis.timingValue.TimingValueLineChart;
 ...
 * @author:fgq
 */
public class TimingMaximinTab30 {
	private static TimingMaximinTab30 tab;
	...
	private JScrollPane scrlTimingMaximin30;// 主经济数据表
	...
	private SSplitPane spltEconimicDataAndNews;
	private JScrollPane scrlChart;
	private JScrollPane scrlKlineChart;
	private STabbedPane tabChart;// 相关经济数据表与K线图
	private Map<String, String> mapQueryParam;

	public static TimingMaximinTab30 getInstance() {
		if (tab == null)
			tab = new TimingMaximinTab30();
		return tab;
	}

	public TimingMaximinTab30() {
		splMain = new SSplitPane(0, 0.3, false);
		spltTop = new SSplitPane(0, 35, false);

		scrlTimingMaximin30 = new JScrollPane();
		scrlEconominDataRelationTabel = new JScrollPane();
		scrlNewsTabel = new JScrollPane();

		spltTop.add(getPnlTop(), JSplitPane.TOP);// 按钮工具栏
		spltTop.add(scrlTimingMaximin30, JSplitPane.BOTTOM);// 主表
		spltEconimicDataAndNews = new SSplitPane(0, 0.5, false);
		spltEconimicDataAndNews.add(scrlEconominDataRelationTabel, JSplitPane.TOP);// 相关经济数据表
		spltEconimicDataAndNews.add(scrlNewsTabel, JSplitPane.BOTTOM);// 新闻
		splMain.add(spltTop, JSplitPane.TOP);
		tabChart = new STabbedPane(new String[] { "相关数据", "时点价差图", "K线图" });
		splMain.add(tabChart, JSplitPane.BOTTOM);// 相关经济数据表、新闻
		tabChart.addTab("相关数据", ImageContext.TabTask, spltEconimicDataAndNews, "相关数据", false);
		scrlChart = new JScrollPane();
		tabChart.addTab("时点价差图", ImageContext.TabTask, scrlChart, "时点价差图", false);
		scrlKlineChart = new JScrollPane();
		tabChart.addTab("K线图", ImageContext.TabTask, scrlKlineChart, "K线图", false);
	...
	public void queryByParams() {
		String tableName = "sa_timingmaximin30";
		...
		StringBuilder sbSql = new StringBuilder(
				"select  distinct a.*,b.country, b.indicatorid,b.indicator indicatorName, b.compareresult,b.predictedresult, b.source,b.publishDate,b.publishTime  from ")
				.append(tableName).append(" a,economic_data b  where  a.economicDataId=b.id and symbol='" + symbol + "'");
		...
		sbSql.append(" order by country,b.publishDate desc ,b.publishTime ,b.indicator");
		JTable table = new TimingMaximinTable30(sbSql.toString(), symbol).getJtable();
		{
			scrlTimingMaximin30.setViewportView(table);
		}
	}
	...
	private void query() {
		...
		String country = UtilString.isNil(cmbCountry.getSelectedItem().toString());
		String tableName = "sa_timingmaximin30";
		...
		StringBuilder sbSql = new StringBuilder(
				"select   distinct a.*,b.country, b.indicatorid,b.indicator indicatorName, b.compareresult,b.predictedresult, b.source,b.publishDate,b.publishTime  from ")
				.append(tableName).append(" a,economic_data b where  a.economicDataId=b.id   and  publishdate >='").append(begDate).append(
						"' and publishdate <='").append(endDate).append("' and a.symbol='").append(symbol).append("'");
		String queryTime = cmbQueryTime.getSelectedItem().toString();
		if (!"".equals(queryTime)) {
			sbSql.append(" and  publishdate='").append(queryTime).append("' ");
		}
		String indicatorName = cmbIndicator.getSelectedItem().toString();
		if (!"".equals(indicatorName) && !"".equals("全部")) {
			sbSql.append(" and b.indicator like'%").append(indicatorName).append("%'");
		}

		if (!"".equals(country) && !"".equals("全部")) {
			sbSql.append(" and b.country like'%").append(country).append("%'");
		}
		sbSql.append(" order by country,b.publishDate desc ,b.publishTime ,b.indicator");
		JTable table = new TimingMaximinTable30(sbSql.toString(), symbol).getJtable();
		{
	...
	public void setChart(String economicDataId, String symbol, String date, String time) {
		scrlChart.setViewportView(new TimingValueLineChart().getChartPanel(economicDataId, symbol, date, time));
	}

	public void setKline(JComponent kLine) {
		// tabChart.remove("K线图");
		// tabChart.addTab("K线图", ImageContext.TabTask, kLine, "K线图", false);
		scrlKlineChart.setViewportView(kLine);
	}
