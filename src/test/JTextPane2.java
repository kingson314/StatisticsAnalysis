...
public class JTextPane2 extends JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -6721614437465565219L;
	...
	public JTextPane2() {
		...
		insertButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				mypane.insertIcon(new ImageIcon(chooser.getSelectedFile()
						.toString()));
			}
		});
		...
		sendButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < mypane.getStyledDocument()
						.getRootElements()[0].getElement(0).getElementCount(); i++) {
					Icon icon = StyleConstants.getIcon(mypane
				...
				for (int i = 0; i < mypane.getText().length(); i++) {
					if (mypane.getStyledDocument().getCharacterElement(i)
							.getName().equals("icon")) {
						System.err.println("你在第 " + i + " 行插入了圖片，圖片的路徑是 :");
						pane.insertIcon(new ImageIcon(picVector.get(k)
								.toString()));
						...
						try {
							pane.getStyledDocument().insertString(
									pane.getText().length(),
									mypane.getStyledDocument().getText(i, 1),
									null);
